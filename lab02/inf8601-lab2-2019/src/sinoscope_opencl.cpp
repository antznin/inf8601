...
int create_buffer(int width, int height)
{
     ...
     */
    cl_int ret = 0;
	output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 3 * width * height *
			sizeof(cl_char), NULL, &ret);
...
int sinoscope_image_opencl(sinoscope_t *ptr)
{
    ...
    cl_event ev;
	sinoscope sino = *ptr;
	size_t work_dim[2] = {(size_t)sino.width, (size_t)sino.height};

	ret = clSetKernelArg(kernel, 0, sizeof(output), &(output))
		| clSetKernelArg(kernel, 1, sizeof(int),    &(sino.width))
		| clSetKernelArg(kernel, 2, sizeof(int),    &(sino.taylor))
		| clSetKernelArg(kernel, 3, sizeof(int),    &(sino.interval))
		| clSetKernelArg(kernel, 4, sizeof(float),  &(sino.interval_inv))
		| clSetKernelArg(kernel, 5, sizeof(float),  &(sino.time))
		| clSetKernelArg(kernel, 6, sizeof(float),  &(sino.phase0))
		| clSetKernelArg(kernel, 7, sizeof(float),  &(sino.phase1))
		| clSetKernelArg(kernel, 8, sizeof(float),  &(sino.dx))
		| clSetKernelArg(kernel, 9, sizeof(float),  &(sino.dy));
